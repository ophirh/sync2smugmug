...
from sync2smugmug import disk, event_manager, events, models
...
async def upload_folder(event_data: events.FolderEventData, dry_run: bool) -> bool:
    ...
    for sub_folder in event_data.source_folder.sub_folders.values():
        ...
        await event_manager.fire_event(event=events.OnlineEventGroup.FOLDER_ADD, event_data=event_data, dry_run=dry_run)
    ...
    for album in event_data.source_folder.albums.values():
        ...
        await event_manager.fire_event(event=events.OnlineEventGroup.ALBUM_ADD, event_data=event_data, dry_run=dry_run)
...
async def download_folder(event_data: events.FolderEventData, dry_run: bool):
    ...
    for sub_folder in event_data.source_folder.sub_folders.values():
        ...
        await event_manager.fire_event(event=events.DiskEventGroup.FOLDER_ADD, event_data=event_data, dry_run=dry_run)
    ...
    for album in event_data.source_folder.albums.values():
        ...
        await event_manager.fire_event(event=events.DiskEventGroup.ALBUM_ADD, event_data=event_data, dry_run=dry_run)
...
async def download_album(event_data: events.AlbumEventData, dry_run: bool) -> bool:
    ...
    disk_info = disk.create_album_disk_info(
        parent_disk_path=disk_parent_folder.disk_info.disk_path, album_name=online_album.name, dry_run=dry_run
    )
    ...
    changed = await online.download_missing_images(
        from_online_album=online_album, to_disk_album=disk_album, connection=event_data.connection, dry_run=dry_run
    )
