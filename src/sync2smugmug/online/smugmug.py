...
class SmugmugCoreConnection:
    ...
    async def request_post(self, relative_uri: str, json_data: dict | list, *args, **kwargs) -> dict:
        ...
        def sync_post() -> httpx.Response:
            return self._sync_session.post(self._format_url(relative_uri), *args, json=json_data, **kwargs)
    ...
    async def paginate(
        ...
        while total_count > items_found:
            response = await self.request_get(relative_uri, params={"start": items_found + 1, "count": page_size})
