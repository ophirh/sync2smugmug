...
class OnlineConnection:
    ...
    async def create_folder(
        self, parent: models.Folder, folder_name: str, dry_run: bool
    ...
    async def upload_images(self, image_paths: list[pathlib.Path], to_album_uri: str, dry_run: bool):
        ...
        for image_path in image_paths:
            await self._conn.request_upload(
                image_path=image_path, album_uri=to_album_uri, image_name=image_path.name, dry_run=dry_run
            )
...
async def download_missing_images(
    from_online_album: models.Album, to_disk_album: models.Album, connection: OnlineConnection, dry_run: bool
) -> bool:
    ...
    await connection.download_images(
        images=(i.online_info for i in missing_images), to_folder=to_disk_album.disk_info.disk_path, dry_run=dry_run
    )
...
async def upload_missing_images(
    from_disk_album: models.Album, to_online_album: models.Album, connection: OnlineConnection, dry_run: bool
) -> bool:
    ...
    await connection.upload_images(
        image_paths=images_to_upload, to_album_uri=to_online_album.online_info.uri, dry_run=dry_run
    )
