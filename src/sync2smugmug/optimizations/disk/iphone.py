        ...
        for photo in photos_db.photos(from_date=last_import_date):
            ...
            if was_created:
                ...
                parent_or_parent = node_tools.get_folder(
                    root_folder=on_disk, relative_path=parent_folder.relative_path.parent
                )
                parent_or_parent.sub_folders[parent_folder.relative_path.name] = parent_folder

            album, was_created = find_or_create_album(
                photo=photo, parent_folder=parent_folder, albums_by_date=albums_by_date
            )
            ...
            export_result = exporter.export(dest=album.disk_info.disk_path, filename=target_filename, options=options)
...
def find_or_create_album(
    photo: osxphotos.PhotoInfo, parent_folder: models.Folder, albums_by_date: dict[date, list[models.Album]]
