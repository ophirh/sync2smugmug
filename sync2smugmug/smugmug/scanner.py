...
class SmugmugScanner:
    ...
    async def scan(self) -> FolderOnSmugmug:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(
                f"Scanning SmugMug (starting from {self._connection.root_folder_uri})..."
            )

        return await self._scan(
            node_uri=None, path=Node.ROOT, parent=None, connection=self._connection
        )
    ...
    async def _scan(
        ...
        folder_record, sub_folder_records, album_records = await self._folder_get(
            folder_uri=node_uri, with_children=True
        )

        folder = FolderOnSmugmug(
            parent=parent,
            relative_path=path,
    ...
    async def _folder_get(
        self, folder_uri: str = None, with_children: bool = True
