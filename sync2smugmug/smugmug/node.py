...
class OnSmugmug:
    def __init__(self, record: Dict, parent: Optional['FolderOnSmugmug'], connection: SmugMugConnection = None):
...
class FolderOnSmugmug(Folder, OnSmugmug):
    ...
    async def upload(self,
        ...
        if from_disk_node.is_folder:
            new_node_record = \
                await self._folder_create(parent_folder=self,
    ...
    async def _folder_create(self,
        ...
        r = await self.connection.request_post(post_url,
                                               json={
    ...
    async def _album_create(self,
        ...
        r = await self.connection.request_post(f'{node_url}!children',
                                               json={
                                                   'Name': album_on_disk.name,
                                                   # 'UrlName': self._encode_uri_name(album_on_disk.name),
                                                   # 'Privacy': 'Unlisted',
...
class AlbumOnSmugmug(Album, OnSmugmug):
    ...
    async def get_images(self) -> List[ImageOnSmugmug]:
        if self._images is None:
            ...
            self._images = [
                ImageOnSmugmug(album=self, record=image)
    ...
    async def upload_images(self, from_album_on_disk: AlbumOnDisk, dry_run: bool):
        ...
        missing_images = [i for p, i in disk_images_dict.items() if p not in smugmug_images]
        if missing_images:
            logger.info(f'Uploading {len(missing_images)} missing images from {from_album_on_disk} to {self}')
        ...
        for smugmug_relative_path, disk_image in disk_images_dict.items():
            ...
            if smugmug_image and disk_image.compare(smugmug_image) > 0:  # Only if disk is 'larger' than smugmug
                ...
                if not dry_run:
                    # noinspection PyTypeChecker
                    tasks.append(asyncio.create_task(self.image_upload(image_on_disk=disk_image,
    ...
    async def download_images(self, to_album_on_disk: AlbumOnDisk, dry_run: bool):
        ...
        disk_images = {i.smugmug_relative_path: i for i in await to_album_on_disk.get_images()}
        smugmug_images = await self.get_images()
        missing_images = {i for i in smugmug_images if i.relative_path not in disk_images}

        if missing_images:
            logger.info(f'Preparing to download {len(missing_images)} images from {self}...')

            if not dry_run:
                tasks = [
                    asyncio.create_task(
                        self.image_download(to_album=to_album_on_disk, image_on_smugmug=image)
                    )
                    for image in missing_images
                ]
    ...
    async def image_download(self,
        ...
        await self.connection.request_download(image_uri=await self.get_download_url(image_on_smugmug),
    ...
    async def get_download_url(self, image_on_smugmug: ImageOnSmugmug) -> str:
        ...
        if image_on_smugmug.is_video and 'LargestVideo' in image_on_smugmug.record['Uris']:
            ...
            r = await self.connection.request_get(image_on_smugmug.record['Uris']['LargestVideo']['Uri'])
    ...
    async def image_upload(self,
        ...
        try:
            ...
            await self.connection.request_upload(album_uri=self.album_uri,
